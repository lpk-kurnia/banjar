...
export async function getCurrentUser(): Promise<AuthUser | null> {
  try {
    ...
    // Try Prisma first with 5-second timeout (prevents hanging on connection pooler)
    try {
      const prismaResult = await Promise.race([
        (async () => {
          ...
          // Auto-create User row if completely missing
          if (!dbUser && user.email) {
      ...
      if (prismaResult) {
        return {
          ...
          avatar: prismaResult.avatar,
          role: prismaResult.role,
          isBanned: prismaResult.isBanned,
        }
      }
    } catch {
      // Prisma failed or timed out — fall back to service_role RPC
    }
